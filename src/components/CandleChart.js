  ...
  const fetchChartData = async () => {
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/api/analytics/get_candle_data?coin=XBT`
      );
    ...
    <div className="chart-container" style={{ width: "100%", height: 500 }}>
      <ResponsiveContainer>
        ...
        >
          ...
          <Bar dataKey="open" fill="#82ca9d" name="Open Price" barSize={15} />
          <Bar dataKey="close" fill="#ff7300" name="Close Price" barSize={15} />
        </ComposedChart>
      </ResponsiveContainer>
    </div>
  );
